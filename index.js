...
const io = require("socket.io")(server, {
    origins: "localhost:8080 hang-in-there.herokuapp.com:*",
});
...
io.on("connection", (socket) => {
    ...
    socket.on("privateMessages", (otherUserId) => {
        ...
        getPrivateMessages(userId, otherUserId)
            ...
            .catch((error) =>
                console.log("error in getPrivateMessages:", error)
            );
    });
    ...
    socket.on("privateMessage", (privateMsg) => {
        ...
        Promise.all([
            insertPrivateMessage(message, userId, receiver_id),
            getUserById(userId),
        ])
            .then((results) => {
